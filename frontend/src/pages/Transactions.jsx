...
export default function Transactions() {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const ignoredKeys = ['date', 'voucher number', 'description', 'discription', 'code'];

        for (const row of data) {
          const date = row['Date'];
          const sn = row['Voucher Number']?.toString();
          const final_description = row['Description'] || row['Discription'] || '';

          if (!date || !sn) continue; // Skip empty rows or rows without mandatory fields
          ...
          for (const key of Object.keys(row)) {
            ...
            if (!isNaN(value) && value !== 0) {
              const codeExists = cachedCodes.some(c => c.code_number.toString() === key.toString());
              if (!codeExists) {
                Swal.fire('Unknown Code', `Account code '${key}' used in the Excel file is not found in the Chart of Accounts.`, 'error');
                e.target.value = ''; // Reset input
                return;
              }

              const amount = Math.abs(value);
              const type = value > 0 ? 'Dr' : 'Cr';

              if (type === 'Dr') totalDr += amount;
              else totalCr += amount;

              lines.push({
                code_number: key.toString(),
                type,
                amount
              });
            }
          ...
          // Check for balanced voucher (with a small epsilon for floating point issues)
          if (Math.abs(totalDr - totalCr) > 0.01) {
            Swal.fire('Unbalanced Voucher', `Voucher Number ${sn} is unbalanced. Dr: ${totalDr.toFixed(2)}, Cr: ${totalCr.toFixed(2)}.`, 'error');
            e.target.value = ''; // Reset input
            return;
          }
        ...
        if (transactionsToPost.length === 0) {
          Swal.fire('Error', 'No valid transactions found in Excel file.', 'error');
          return;
        }
        ...
        const res = await fetch('/api/transactions/bulk', {
          ...
          body: JSON.stringify({ transactions: transactionsToPost })
        });

        if (res.ok) {
          const result = await res.json();
          Swal.fire('Success', result.message || `Uploaded ${transactionsToPost.length} vouchers.`, 'success').then(() => {
            navigate(0); // refresh the page
          });
        } else {
          const err = await res.json();
          Swal.fire('Upload Failed', err.error, 'error');
        }
  ...
  const handleSubmit = async (e) => {
    ...
    const rawLines = lines.flatMap(l => {
      const result = [];
